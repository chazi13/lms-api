...
try {
    ...
    externalHook = require(path + '/hooks/chatFileStorage')
} catch (e) {
...
const chatFileStorageService = new cote.Responder({
    name: 'ChatFileStorage Service',
    key: 'chatFileStorage'
})
...
chatFileStorageService.on("find", async (req, cb) => {
    try {
        let data = await app.service("chatFileStorages").find({
...
chatFileStorageService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("chatFileStorages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
chatFileStorageService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("chatFileStorages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
chatFileStorageService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("chatFileStorages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
chatFileStorageService.on("create", async (req, cb) => {
    try {
        let data = await app.service("chatFileStorages").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
chatFileStorageService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("chatFileStorages").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
chatFileStorageService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("chatFileStorages").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
chatFileStorageService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("chatFileStorages").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('chatFileStorages').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('chatFileStorage')}:findOwn`)){
        ...
        create: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeCreate
                    if(context.data && context.data.messageId){
                        let belongsTo = await getRequester('message').send({ 
                            type: "get", 
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('chatFileStorage')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let chatFileStorage = await app.service(`${pluralize(camelize("chatFileStorage"))}`).get(context.id, { headers: context.params.headers })
                            if(chatFileStorage && chatFileStorage.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('chatFileStorage')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let chatFileStorage = await app.service(`${pluralize(camelize("chatFileStorages"))}`).get(context.id, { headers: context.params.headers })
                            if(chatFileStorage && chatFileStorage.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('chatFileStorage')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('ChatFileStorage Rest Server on http://%s:%d', app.get('host'), port)
);
