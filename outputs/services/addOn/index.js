...
try {
    ...
    externalHook = require(path + '/hooks/addOn')
} catch (e) {
...
const addOnService = new cote.Responder({
    name: 'AddOn Service',
    key: 'addOn'
})
...
addOnService.on("find", async (req, cb) => {
    try {
...
addOnService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("addOns").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
addOnService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("addOns").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
addOnService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("addOns").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
addOnService.on("create", async (req, cb) => {
    try {
...
addOnService.on("patch", async (req, cb) => {
    try {
...
addOnService.on("delete", async (req, cb) => {
    try {
...
addOnService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('addOn')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('addOn')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let addOn = await app.service(`${pluralize(camelize("addOn"))}`).get(context.id, { headers: context.params.headers })
                            if(addOn && addOn.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('addOn')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let addOn = await app.service(`${pluralize(camelize("addOns"))}`).get(context.id, { headers: context.params.headers })
                            if(addOn && addOn.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('addOn')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('AddOn Rest Server on http://%s:%d', app.get('host'), port)
);
