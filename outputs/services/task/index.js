...
try {
    ...
    externalHook = require(path + '/hooks/task')
} catch (e) {
...
const taskService = new cote.Responder({
    name: 'Task Service',
    key: 'task'
})
...
taskService.on("find", async (req, cb) => {
    try {
        let data = await app.service("tasks").find({
...
taskService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("tasks").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
taskService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("tasks").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
taskService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("tasks").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
taskService.on("create", async (req, cb) => {
    try {
        let data = await app.service("tasks").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
taskService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("tasks").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
taskService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("tasks").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
taskService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("tasks").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('task')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('task')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let task = await app.service(`${pluralize(camelize("task"))}`).get(context.id, { headers: context.params.headers })
                            if(task && task.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('task')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let task = await app.service(`${pluralize(camelize("tasks"))}`).get(context.id, { headers: context.params.headers })
                            if(task && task.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('task')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Task Rest Server on http://%s:%d', app.get('host'), port)
);
