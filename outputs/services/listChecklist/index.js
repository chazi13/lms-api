...
try {
    ...
    externalHook = require(path + '/hooks/listChecklist');
} catch (e) {
...
const listChecklistService = new cote.Responder({
    name: 'ListChecklist Service',
...
listChecklistService.on("find", async (req, cb) => {
    try {
        let data = await app.service("listChecklists").find({
...
listChecklistService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("listChecklists").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
listChecklistService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("listChecklists").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
listChecklistService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("listChecklists").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
listChecklistService.on("create", async (req, cb) => {
    try {
        let data = await app.service("listChecklists").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
listChecklistService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("listChecklists").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
listChecklistService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("listChecklists").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
listChecklistService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("listChecklists").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('listChecklists').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('listChecklist')}:findOwn`)){
        ...
        create: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeCreate
                    if(context.data && context.data.checklistId){
                        let belongsTo = await getRequester('checklist').send({ 
                            type: "get", 
                            id: context.data.checklistId, 
                            headers:{
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('listChecklist')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let listChecklist = await app.service(`${pluralize(camelize("listChecklist"))}`).get(context.id, { headers: context.params.headers })
                            if(listChecklist && listChecklist.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('listChecklist')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let listChecklist = await app.service(`${pluralize(camelize("listChecklists"))}`).get(context.id, { headers: context.params.headers })
                            if(listChecklist && listChecklist.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('listChecklist')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('ListChecklist Rest Server on http://%s:%d', app.get('host'), port)
);
