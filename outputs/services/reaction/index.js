...
try {
    ...
    externalHook = require(path + '/hooks/reaction');
} catch (e) {
...
const reactionService = new cote.Responder({
...
reactionService.on("find", async (req, cb) => {
    try {
        let data = await app.service("reactions").find({
...
reactionService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("reactions").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
reactionService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("reactions").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
reactionService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("reactions").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
reactionService.on("create", async (req, cb) => {
    try {
        let data = await app.service("reactions").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
reactionService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("reactions").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
reactionService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("reactions").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
reactionService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("reactions").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('reaction')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('reaction')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let reaction = await app.service(`${pluralize(camelize("reaction"))}`).get(context.id, { headers: context.params.headers })
                            if(reaction && reaction.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('reaction')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let reaction = await app.service(`${pluralize(camelize("reactions"))}`).get(context.id, { headers: context.params.headers })
                            if(reaction && reaction.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('reaction')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Reaction Rest Server on http://%s:%d', app.get('host'), port)
);
