...
try {
    ...
    externalHook = require(path + '/hooks/review');
} catch (e) {
...
const reviewService = new cote.Responder({
    name: 'Review Service',
    key: APP_ID + '_review'
})
...
reviewService.on("find", async (req, cb) => {
    try {
        let data = await app.service("reviews").find({
...
reviewService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("reviews").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
reviewService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("reviews").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
reviewService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("reviews").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
reviewService.on("create", async (req, cb) => {
    try {
        let data = await app.service("reviews").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
reviewService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("reviews").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
reviewService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("reviews").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
reviewService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("reviews").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('review')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('review')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let review = await app.service(`${pluralize(camelize("review"))}`).get(context.id, { headers: context.params.headers })
                            if(review && review.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('review')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let review = await app.service(`${pluralize(camelize("reviews"))}`).get(context.id, { headers: context.params.headers })
                            if(review && review.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('review')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Review Rest Server on http://%s:%d', app.get('host'), port)
);
