...
try {
    ...
    externalHook = require(path + '/hooks/work')
} catch (e) {
...
const workService = new cote.Responder({
    name: 'Work Service',
    key: 'work'
})
...
workService.on("find", async (req, cb) => {
    try {
        let data = await app.service("works").find({
...
workService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("works").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
workService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("works").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
workService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("works").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
workService.on("create", async (req, cb) => {
    try {
        let data = await app.service("works").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
workService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("works").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
workService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("works").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
workService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("works").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('work')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('work')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let work = await app.service(`${pluralize(camelize("work"))}`).get(context.id, { headers: context.params.headers })
                            if(work && work.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('work')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let work = await app.service(`${pluralize(camelize("works"))}`).get(context.id, { headers: context.params.headers })
                            if(work && work.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('work')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Work Rest Server on http://%s:%d', app.get('host'), port)
);
