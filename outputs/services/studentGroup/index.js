...
try {
    ...
    externalHook = require(path + '/hooks/studentGroup')
} catch (e) {
...
const studentGroupService = new cote.Responder({
    name: 'StudentGroup Service',
    key: 'studentGroup'
})
...
studentGroupService.on("find", async (req, cb) => {
    try {
        let data = await app.service("studentGroups").find({
...
studentGroupService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("studentGroups").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentGroupService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("studentGroups").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentGroupService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("studentGroups").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentGroupService.on("create", async (req, cb) => {
    try {
        let data = await app.service("studentGroups").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
studentGroupService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("studentGroups").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
studentGroupService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("studentGroups").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
studentGroupService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("studentGroups").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('studentGroups').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('studentGroup')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('studentGroup')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let studentGroup = await app.service(`${pluralize(camelize("studentGroup"))}`).get(context.id, { headers: context.params.headers })
                            if(studentGroup && studentGroup.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('studentGroup')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let studentGroup = await app.service(`${pluralize(camelize("studentGroups"))}`).get(context.id, { headers: context.params.headers })
                            if(studentGroup && studentGroup.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('studentGroup')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('StudentGroup Rest Server on http://%s:%d', app.get('host'), port)
);
