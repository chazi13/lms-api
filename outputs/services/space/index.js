...
try {
    ...
    externalHook = require(path + '/hooks/space');
} catch (e) {
...
const spaceService = new cote.Responder({
    name: 'Space Service',
    key: APP_ID + '_space'
})
...
spaceService.on("find", async (req, cb) => {
    try {
        let data = await app.service("spaces").find({
...
spaceService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("spaces").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
spaceService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("spaces").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
spaceService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("spaces").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
spaceService.on("create", async (req, cb) => {
    try {
        let data = await app.service("spaces").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
spaceService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("spaces").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
spaceService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("spaces").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
spaceService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("spaces").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('space')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('space')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let space = await app.service(`${pluralize(camelize("space"))}`).get(context.id, { headers: context.params.headers })
                            if(space && space.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('space')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let space = await app.service(`${pluralize(camelize("spaces"))}`).get(context.id, { headers: context.params.headers })
                            if(space && space.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('space')}:removeOwn`)){
                    ...
                    await getRequester('workspace').send({ type: 'delete', 
                        id: null,   
                        headers: {
                    ...
                    //ON DELETE SET CASCADE
                    await getRequester('checkInRoom').send({ type: 'delete', 
                        id: null,   
                        headers: {
                            authorization: context.params.headers.authorization
                        }, 
                    ...
                    //ON DELETE SET CASCADE
                    await getRequester('post').send({ type: 'delete', 
                        id: null,   
                        headers: {
                            authorization: context.params.headers.authorization
                        }, 
                    ...
                    //ON DELETE SET CASCADE
                    await getRequester('event').send({ type: 'delete', 
                        id: null,   
                        headers: {
                            authorization: context.params.headers.authorization
                        }, 
                    ...
                    //ON DELETE SET CASCADE
                    await getRequester('folder').send({ type: 'delete', 
                        id: null,   
                        headers: {
                            authorization: context.params.headers.authorization
                        }, 
                    ...
                    //ON DELETE SET CASCADE
                    await getRequester('course').send({ type: 'delete', 
                        id: null,   
                        headers: {
                            authorization: context.params.headers.authorization
                        }, 
...
server.on('listening', () =>
    console.log('Space Rest Server on http://%s:%d', app.get('host'), port)
);
