...
try {
    ...
    externalHook = require(path + '/hooks/article');
} catch (e) {
...
const articleService = new cote.Responder({
...
articleService.on("find", async (req, cb) => {
    try {
        let data = await app.service("articles").find({
...
articleService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("articles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
articleService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("articles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
articleService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("articles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
articleService.on("create", async (req, cb) => {
    try {
        let data = await app.service("articles").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
articleService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("articles").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
articleService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("articles").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
articleService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("articles").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('article')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('article')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let article = await app.service(`${pluralize(camelize("article"))}`).get(context.id, { headers: context.params.headers })
                            if(article && article.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('article')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let article = await app.service(`${pluralize(camelize("articles"))}`).get(context.id, { headers: context.params.headers })
                            if(article && article.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('article')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Article Rest Server on http://%s:%d', app.get('host'), port)
);
