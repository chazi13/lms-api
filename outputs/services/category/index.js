...
try {
    ...
    externalHook = require(path + '/hooks/category')
} catch (e) {
...
const categoryService = new cote.Responder({
...
categoryService.on("find", async (req, cb) => {
    try {
        let data = await app.service("categories").find({
...
categoryService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("categories").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
categoryService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("categories").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
categoryService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("categories").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
categoryService.on("create", async (req, cb) => {
    try {
        let data = await app.service("categories").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
categoryService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("categories").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
categoryService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("categories").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
categoryService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("categories").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('category')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('category')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let category = await app.service(`${pluralize(camelize("category"))}`).get(context.id, { headers: context.params.headers })
                            if(category && category.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('category')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let category = await app.service(`${pluralize(camelize("categories"))}`).get(context.id, { headers: context.params.headers })
                            if(category && category.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('category')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Category Rest Server on http://%s:%d', app.get('host'), port)
);
