...
try {
    ...
    externalHook = require(path + '/hooks/image')
} catch (e) {
...
imageService.on("find", async (req, cb) => {
    try {
        let data = await app.service("images").find({
...
imageService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("images").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
imageService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("images").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
imageService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("images").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
imageService.on("create", async (req, cb) => {
    try {
        let data = await app.service("images").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
imageService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("images").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
imageService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("images").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
imageService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("images").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('image')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('image')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let image = await app.service(`${pluralize(camelize("image"))}`).get(context.id, { headers: context.params.headers })
                            if(image && image.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('image')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let image = await app.service(`${pluralize(camelize("images"))}`).get(context.id, { headers: context.params.headers })
                            if(image && image.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('image')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Image Rest Server on http://%s:%d', app.get('host'), port)
);
