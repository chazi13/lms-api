...
try {
    ...
    externalHook = require(path + '/hooks/education')
} catch (e) {
...
const educationService = new cote.Responder({
...
educationService.on("find", async (req, cb) => {
    try {
        let data = await app.service("educations").find({
...
educationService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("educations").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
educationService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("educations").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
educationService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("educations").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
educationService.on("create", async (req, cb) => {
    try {
        let data = await app.service("educations").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
educationService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("educations").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
educationService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("educations").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
educationService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("educations").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('education')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('education')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let education = await app.service(`${pluralize(camelize("education"))}`).get(context.id, { headers: context.params.headers })
                            if(education && education.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('education')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let education = await app.service(`${pluralize(camelize("educations"))}`).get(context.id, { headers: context.params.headers })
                            if(education && education.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('education')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Education Rest Server on http://%s:%d', app.get('host'), port)
);
