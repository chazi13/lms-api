...
try {
    ...
    externalHook = require(path + '/hooks/studentWorkspace')
} catch (e) {
...
const studentWorkspaceService = new cote.Responder({
    name: 'StudentWorkspace Service',
...
studentWorkspaceService.on("find", async (req, cb) => {
    try {
        let data = await app.service("studentWorkspaces").find({
...
studentWorkspaceService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("studentWorkspaces").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentWorkspaceService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("studentWorkspaces").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentWorkspaceService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("studentWorkspaces").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentWorkspaceService.on("create", async (req, cb) => {
    try {
        let data = await app.service("studentWorkspaces").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
studentWorkspaceService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("studentWorkspaces").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
studentWorkspaceService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("studentWorkspaces").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
studentWorkspaceService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("studentWorkspaces").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('studentWorkspaces').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('studentWorkspace')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('studentWorkspace')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let studentWorkspace = await app.service(`${pluralize(camelize("studentWorkspace"))}`).get(context.id, { headers: context.params.headers })
                            if(studentWorkspace && studentWorkspace.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('studentWorkspace')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let studentWorkspace = await app.service(`${pluralize(camelize("studentWorkspaces"))}`).get(context.id, { headers: context.params.headers })
                            if(studentWorkspace && studentWorkspace.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('studentWorkspace')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('StudentWorkspace Rest Server on http://%s:%d', app.get('host'), port)
);
