...
try {
    ...
    externalHook = require(path + '/hooks/question');
} catch (e) {
...
const questionService = new cote.Responder({
...
questionService.on("find", async (req, cb) => {
    try {
        let data = await app.service("questions").find({
...
questionService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("questions").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
questionService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("questions").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
questionService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("questions").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
questionService.on("create", async (req, cb) => {
    try {
        let data = await app.service("questions").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
questionService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("questions").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
questionService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("questions").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
questionService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("questions").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('question')}:findOwn`)){
        ...
        create: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeCreate
                    if(context.data && context.data.quizId){
                        let belongsTo = await getRequester('quiz').send({ 
                            type: "get", 
                            id: context.data.quizId, 
                            headers:{
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('question')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let question = await app.service(`${pluralize(camelize("question"))}`).get(context.id, { headers: context.params.headers })
                            if(question && question.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('question')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let question = await app.service(`${pluralize(camelize("questions"))}`).get(context.id, { headers: context.params.headers })
                            if(question && question.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('question')}:removeOwn`)){
                    ...
                    await getRequester('option').send({ type: 'delete', 
                        id: null,   
                        headers: {
...
server.on('listening', () =>
    console.log('Question Rest Server on http://%s:%d', app.get('host'), port)
);
