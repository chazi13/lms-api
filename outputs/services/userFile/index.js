...
try {
    ...
    externalHook = require(path + '/hooks/userFile')
} catch (e) {
...
const userFileService = new cote.Responder({
    name: 'UserFile Service',
    key: 'userFile'
})
...
userFileService.on("find", async (req, cb) => {
    try {
        let data = await app.service("userFiles").find({
...
userFileService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("userFiles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
userFileService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("userFiles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
userFileService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("userFiles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
userFileService.on("create", async (req, cb) => {
    try {
        let data = await app.service("userFiles").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
userFileService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("userFiles").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
userFileService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("userFiles").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
userFileService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("userFiles").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('userFile')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('userFile')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let userFile = await app.service(`${pluralize(camelize("userFile"))}`).get(context.id, { headers: context.params.headers })
                            if(userFile && userFile.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('userFile')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let userFile = await app.service(`${pluralize(camelize("userFiles"))}`).get(context.id, { headers: context.params.headers })
                            if(userFile && userFile.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('userFile')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('UserFile Rest Server on http://%s:%d', app.get('host'), port)
);
