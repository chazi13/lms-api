...
try {
    ...
    externalHook = require(path + '/hooks/board');
} catch (e) {
...
const boardService = new cote.Responder({
    name: 'Board Service',
    key: APP_ID + '_board'
})
...
boardService.on("find", async (req, cb) => {
    try {
        let data = await app.service("boards").find({
...
boardService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("boards").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
boardService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("boards").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
boardService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("boards").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
boardService.on("create", async (req, cb) => {
    try {
        let data = await app.service("boards").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
boardService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("boards").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
boardService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("boards").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
boardService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("boards").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('board')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('board')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let board = await app.service(`${pluralize(camelize("board"))}`).get(context.id, { headers: context.params.headers })
                            if(board && board.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('board')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let board = await app.service(`${pluralize(camelize("boards"))}`).get(context.id, { headers: context.params.headers })
                            if(board && board.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('board')}:removeOwn`)){
                    ...
                    let lists = await getRequester('list').send({ 
                        type: 'find', 
                        where: {
                            boardId: context.id
                        }, 
...
server.on('listening', () =>
    console.log('Board Rest Server on http://%s:%d', app.get('host'), port)
);
