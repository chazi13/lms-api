...
try {
    ...
    externalHook = require(path + '/hooks/card');
} catch (e) {
...
const cardService = new cote.Responder({
    name: 'Card Service',
    key: APP_ID + '_card'
})
...
cardService.on("find", async (req, cb) => {
    try {
        let data = await app.service("cards").find({
...
cardService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("cards").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
cardService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("cards").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
cardService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("cards").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
cardService.on("create", async (req, cb) => {
    try {
        let data = await app.service("cards").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
cardService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("cards").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
cardService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("cards").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
cardService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("cards").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('card')}:findOwn`)){
        ...
        create: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeCreate
                    if(context.data && context.data.listId){
                        let belongsTo = await getRequester('list').send({ 
                            type: "get", 
                            id: context.data.listId, 
                            headers:{
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('card')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let card = await app.service(`${pluralize(camelize("card"))}`).get(context.id, { headers: context.params.headers })
                            if(card && card.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('card')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let card = await app.service(`${pluralize(camelize("cards"))}`).get(context.id, { headers: context.params.headers })
                            if(card && card.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('card')}:removeOwn`)){
                    ...
                    await getRequester('label').send({ type: 'delete', 
                        id: null,   
                        headers: {
                    ...
                    //ON DELETE SET CASCADE
                    await getRequester('checklist').send({ type: 'delete', 
                        id: null,   
                        headers: {
                            authorization: context.params.headers.authorization
                        }, 
...
server.on('listening', () =>
    console.log('Card Rest Server on http://%s:%d', app.get('host'), port)
);
