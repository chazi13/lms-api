...
try {
    ...
    externalHook = require(path + '/hooks/userFriend')
} catch (e) {
...
const userFriendService = new cote.Responder({
    name: 'UserFriend Service',
    key: 'userFriend'
})
...
userFriendService.on("find", async (req, cb) => {
    try {
        let data = await app.service("userFriends").find({
...
userFriendService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("userFriends").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
userFriendService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("userFriends").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
userFriendService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("userFriends").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
userFriendService.on("create", async (req, cb) => {
    try {
        let data = await app.service("userFriends").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
userFriendService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("userFriends").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
userFriendService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("userFriends").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
userFriendService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("userFriends").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('userFriend')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('userFriend')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let userFriend = await app.service(`${pluralize(camelize("userFriend"))}`).get(context.id, { headers: context.params.headers })
                            if(userFriend && userFriend.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('userFriend')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let userFriend = await app.service(`${pluralize(camelize("userFriends"))}`).get(context.id, { headers: context.params.headers })
                            if(userFriend && userFriend.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('userFriend')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('UserFriend Rest Server on http://%s:%d', app.get('host'), port)
);
