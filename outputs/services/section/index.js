...
try {
    ...
    externalHook = require(path + '/hooks/section')
} catch (e) {
...
const sectionService = new cote.Responder({
...
sectionService.on("find", async (req, cb) => {
    try {
        let data = await app.service("sections").find({
...
sectionService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("sections").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
sectionService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("sections").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
sectionService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("sections").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
sectionService.on("create", async (req, cb) => {
    try {
        let data = await app.service("sections").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
sectionService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("sections").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
sectionService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("sections").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
sectionService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("sections").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('section')}:findOwn`)){
        ...
        create: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeCreate
                    if(context.data && context.data.courseId){
                        let belongsTo = await getRequester('course').send({ 
                            type: "get", 
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('section')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let section = await app.service(`${pluralize(camelize("section"))}`).get(context.id, { headers: context.params.headers })
                            if(section && section.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('section')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let section = await app.service(`${pluralize(camelize("sections"))}`).get(context.id, { headers: context.params.headers })
                            if(section && section.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('section')}:removeOwn`)){
                    ...
                    let lectures = await getRequester('lecture').send({ 
                        type: 'find', 
                        query: {
...
server.on('listening', () =>
    console.log('Section Rest Server on http://%s:%d', app.get('host'), port)
);
