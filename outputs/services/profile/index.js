...
try {
    ...
    externalHook = require(path + '/hooks/profile')
} catch (e) {
...
const profileService = new cote.Responder({
...
profileService.on("find", async (req, cb) => {
    try {
        let data = await app.service("profiles").find({
...
profileService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("profiles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
profileService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("profiles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
profileService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("profiles").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
profileService.on("create", async (req, cb) => {
    try {
        let data = await app.service("profiles").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
profileService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("profiles").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
profileService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("profiles").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
profileService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("profiles").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('profile')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('profile')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let profile = await app.service(`${pluralize(camelize("profile"))}`).get(context.id, { headers: context.params.headers })
                            if(profile && profile.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('profile')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let profile = await app.service(`${pluralize(camelize("profiles"))}`).get(context.id, { headers: context.params.headers })
                            if(profile && profile.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('profile')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Profile Rest Server on http://%s:%d', app.get('host'), port)
);
