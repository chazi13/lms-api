...
try {
    ...
    externalHook = require(path + '/hooks/skill')
} catch (e) {
...
const skillService = new cote.Responder({
    name: 'Skill Service',
    key: 'skill'
})
...
skillService.on("find", async (req, cb) => {
    try {
        let data = await app.service("skills").find({
...
skillService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("skills").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
skillService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("skills").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
skillService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("skills").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
skillService.on("create", async (req, cb) => {
    try {
        let data = await app.service("skills").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
skillService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("skills").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
skillService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("skills").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
skillService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("skills").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('skill')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('skill')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let skill = await app.service(`${pluralize(camelize("skill"))}`).get(context.id, { headers: context.params.headers })
                            if(skill && skill.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('skill')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let skill = await app.service(`${pluralize(camelize("skills"))}`).get(context.id, { headers: context.params.headers })
                            if(skill && skill.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('skill')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Skill Rest Server on http://%s:%d', app.get('host'), port)
);
