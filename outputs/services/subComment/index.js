...
try {
    ...
    externalHook = require(path + '/hooks/subComment');
} catch (e) {
...
const subCommentService = new cote.Responder({
    name: 'SubComment Service',
...
subCommentService.on("find", async (req, cb) => {
    try {
        let data = await app.service("subComments").find({
...
subCommentService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("subComments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
subCommentService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("subComments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
subCommentService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("subComments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
subCommentService.on("create", async (req, cb) => {
    try {
        let data = await app.service("subComments").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
subCommentService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("subComments").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
subCommentService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("subComments").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
subCommentService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("subComments").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('subComment')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('subComment')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let subComment = await app.service(`${pluralize(camelize("subComment"))}`).get(context.id, { headers: context.params.headers })
                            if(subComment && subComment.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('subComment')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let subComment = await app.service(`${pluralize(camelize("subComments"))}`).get(context.id, { headers: context.params.headers })
                            if(subComment && subComment.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('subComment')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('SubComment Rest Server on http://%s:%d', app.get('host'), port)
);
