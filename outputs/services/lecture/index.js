...
lectureService.on("find", async (req, cb) => {
    try {
        let data = await app.service("lectures").find({
...
lectureService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("lectures").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
lectureService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("lectures").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
lectureService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("lectures").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
lectureService.on("create", async (req, cb) => {
    try {
        let data = await app.service("lectures").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
lectureService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("lectures").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
lectureService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("lectures").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
lectureService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("lectures").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('lecture')}:findOwn`)){
        ...
        create: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(context.data && context.data.articleId){
                        let belongsTo = await getRequester('article').send({ 
                            type: "get", 
                            id: context.data.articleId, 
                            headers:{
                                token: context.params.headers.authorization
                            }
                        })
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('lecture')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let lecture = await app.service(`${pluralize(camelize("lecture"))}`).get(context.id, { headers: context.params.headers })
                            if(lecture && lecture.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('lecture')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let lecture = await app.service(`${pluralize(camelize("lectures"))}`).get(context.id, { headers: context.params.headers })
                            if(lecture && lecture.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('lecture')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Lecture Rest Server on http://%s:%d', app.get('host'), port)
);
