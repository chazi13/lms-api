...
try {
    ...
    externalHook = require(path + '/hooks/post');
} catch (e) {
...
const postService = new cote.Responder({
    name: 'Post Service',
    key: APP_ID + '_post'
})
...
postService.on("find", async (req, cb) => {
    try {
        let data = await app.service("posts").find({
...
postService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("posts").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
postService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("posts").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
postService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("posts").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
postService.on("create", async (req, cb) => {
    try {
        let data = await app.service("posts").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
postService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("posts").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
postService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("posts").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
postService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("posts").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('post')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('post')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let post = await app.service(`${pluralize(camelize("post"))}`).get(context.id, { headers: context.params.headers })
                            if(post && post.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('post')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let post = await app.service(`${pluralize(camelize("posts"))}`).get(context.id, { headers: context.params.headers })
                            if(post && post.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('post')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Post Rest Server on http://%s:%d', app.get('host'), port)
);
