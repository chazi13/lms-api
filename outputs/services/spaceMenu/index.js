...
try {
    ...
    externalHook = require(path + '/hooks/spaceMenu');
} catch (e) {
...
const spaceMenuService = new cote.Responder({
    name: 'SpaceMenu Service',
...
spaceMenuService.on("find", async (req, cb) => {
    try {
        let data = await app.service("spaceMenus").find({
...
spaceMenuService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("spaceMenus").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
spaceMenuService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("spaceMenus").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
spaceMenuService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("spaceMenus").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
spaceMenuService.on("create", async (req, cb) => {
    try {
        let data = await app.service("spaceMenus").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
spaceMenuService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("spaceMenus").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
spaceMenuService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("spaceMenus").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
spaceMenuService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("spaceMenus").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('spaceMenu')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('spaceMenu')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let spaceMenu = await app.service(`${pluralize(camelize("spaceMenu"))}`).get(context.id, { headers: context.params.headers })
                            if(spaceMenu && spaceMenu.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('spaceMenu')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let spaceMenu = await app.service(`${pluralize(camelize("spaceMenus"))}`).get(context.id, { headers: context.params.headers })
                            if(spaceMenu && spaceMenu.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('spaceMenu')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('SpaceMenu Rest Server on http://%s:%d', app.get('host'), port)
);
