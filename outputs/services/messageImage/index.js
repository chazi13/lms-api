...
try {
    ...
    externalHook = require(path + '/hooks/messageImage')
} catch (e) {
...
const messageImageService = new cote.Responder({
    name: 'MessageImage Service',
    key: 'messageImage'
})
...
messageImageService.on("find", async (req, cb) => {
    try {
        let data = await app.service("messageImages").find({
...
messageImageService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("messageImages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
messageImageService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("messageImages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
messageImageService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("messageImages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
messageImageService.on("create", async (req, cb) => {
    try {
        let data = await app.service("messageImages").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
messageImageService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("messageImages").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
messageImageService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("messageImages").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
messageImageService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("messageImages").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('messageImages').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('messageImage')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('messageImage')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let messageImage = await app.service(`${pluralize(camelize("messageImage"))}`).get(context.id, { headers: context.params.headers })
                            if(messageImage && messageImage.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('messageImage')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let messageImage = await app.service(`${pluralize(camelize("messageImages"))}`).get(context.id, { headers: context.params.headers })
                            if(messageImage && messageImage.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('messageImage')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('MessageImage Rest Server on http://%s:%d', app.get('host'), port)
);
