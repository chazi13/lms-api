...
quizService.on("find", async (req, cb) => {
    try {
        let data = await app.service("quizzes").find({
...
quizService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("quizzes").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
quizService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("quizzes").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
quizService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("quizzes").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
quizService.on("create", async (req, cb) => {
    try {
        let data = await app.service("quizzes").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
quizService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("quizzes").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
quizService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("quizzes").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
quizService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("quizzes").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('quiz')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('quiz')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let quiz = await app.service(`${pluralize(camelize("quiz"))}`).get(context.id, { headers: context.params.headers })
                            if(quiz && quiz.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('quiz')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let quiz = await app.service(`${pluralize(camelize("quizzes"))}`).get(context.id, { headers: context.params.headers })
                            if(quiz && quiz.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('quiz')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Quiz Rest Server on http://%s:%d', app.get('host'), port)
);
