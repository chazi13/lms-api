...
try {
    ...
    externalHook = require(path + '/hooks/fileStorage')
} catch (e) {
...
const fileStorageService = new cote.Responder({
...
fileStorageService.on("find", async (req, cb) => {
    try {
        let data = await app.service("fileStorages").find({
...
fileStorageService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("fileStorages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
fileStorageService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("fileStorages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
fileStorageService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("fileStorages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
fileStorageService.on("create", async (req, cb) => {
    try {
        let data = await app.service("fileStorages").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
fileStorageService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("fileStorages").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
fileStorageService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("fileStorages").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
fileStorageService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("fileStorages").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('fileStorages').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('fileStorage')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('fileStorage')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let fileStorage = await app.service(`${pluralize(camelize("fileStorage"))}`).get(context.id, { headers: context.params.headers })
                            if(fileStorage && fileStorage.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('fileStorage')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let fileStorage = await app.service(`${pluralize(camelize("fileStorages"))}`).get(context.id, { headers: context.params.headers })
                            if(fileStorage && fileStorage.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('fileStorage')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('FileStorage Rest Server on http://%s:%d', app.get('host'), port)
);
