...
try {
    ...
    externalHook = require(path + '/hooks/courseCategory')
} catch (e) {
...
const courseCategoryService = new cote.Responder({
    name: 'CourseCategory Service',
    key: 'courseCategory'
})
...
courseCategoryService.on("find", async (req, cb) => {
    try {
        let data = await app.service("courseCategories").find({
...
courseCategoryService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("courseCategories").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
courseCategoryService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("courseCategories").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
courseCategoryService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("courseCategories").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
courseCategoryService.on("create", async (req, cb) => {
    try {
        let data = await app.service("courseCategories").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
courseCategoryService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("courseCategories").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
courseCategoryService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("courseCategories").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
courseCategoryService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("courseCategories").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('courseCategories').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('courseCategory')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('courseCategory')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let courseCategory = await app.service(`${pluralize(camelize("courseCategory"))}`).get(context.id, { headers: context.params.headers })
                            if(courseCategory && courseCategory.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('courseCategory')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let courseCategory = await app.service(`${pluralize(camelize("courseCategories"))}`).get(context.id, { headers: context.params.headers })
                            if(courseCategory && courseCategory.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('courseCategory')}:removeOwn`)){
                    ...
                    let courses = await getRequester('course').send({ 
                        type: 'find', 
                        query: {
                            courseCategoryId: context.id
                        }, 
                        headers: {
...
server.on('listening', () =>
    console.log('CourseCategory Rest Server on http://%s:%d', app.get('host'), port)
);
