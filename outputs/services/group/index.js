...
try {
    ...
    externalHook = require(path + '/hooks/group')
} catch (e) {
...
const groupService = new cote.Responder({
    name: 'Group Service',
    key: 'group'
})
...
groupService.on("find", async (req, cb) => {
    try {
        let data = await app.service("groups").find({
...
groupService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("groups").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
groupService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("groups").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
groupService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("groups").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
groupService.on("create", async (req, cb) => {
    try {
        let data = await app.service("groups").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
groupService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("groups").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
groupService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("groups").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
groupService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("groups").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('group')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('group')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let group = await app.service(`${pluralize(camelize("group"))}`).get(context.id, { headers: context.params.headers })
                            if(group && group.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('group')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let group = await app.service(`${pluralize(camelize("groups"))}`).get(context.id, { headers: context.params.headers })
                            if(group && group.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('group')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Group Rest Server on http://%s:%d', app.get('host'), port)
);
