...
try {
    ...
    externalHook = require(path + '/hooks/studentClass')
} catch (e) {
...
const studentClassService = new cote.Responder({
    name: 'StudentClass Service',
    key: 'studentClass'
})
...
studentClassService.on("find", async (req, cb) => {
    try {
        let data = await app.service("studentClasses").find({
...
studentClassService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("studentClasses").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentClassService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("studentClasses").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentClassService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("studentClasses").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
studentClassService.on("create", async (req, cb) => {
    try {
        let data = await app.service("studentClasses").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
studentClassService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("studentClasses").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
studentClassService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("studentClasses").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
studentClassService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("studentClasses").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('studentClasses').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('studentClass')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('studentClass')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let studentClass = await app.service(`${pluralize(camelize("studentClass"))}`).get(context.id, { headers: context.params.headers })
                            if(studentClass && studentClass.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('studentClass')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let studentClass = await app.service(`${pluralize(camelize("studentClasses"))}`).get(context.id, { headers: context.params.headers })
                            if(studentClass && studentClass.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('studentClass')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('StudentClass Rest Server on http://%s:%d', app.get('host'), port)
);
