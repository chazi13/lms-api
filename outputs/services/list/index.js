...
try {
    ...
    externalHook = require(path + '/hooks/list');
} catch (e) {
...
const listService = new cote.Responder({
    name: 'List Service',
    key: APP_ID + '_list'
})
...
listService.on("find", async (req, cb) => {
    try {
        let data = await app.service("lists").find({
...
listService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("lists").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
listService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("lists").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
listService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("lists").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
listService.on("create", async (req, cb) => {
    try {
        let data = await app.service("lists").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
listService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("lists").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
listService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("lists").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
listService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("lists").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('list')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('list')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let list = await app.service(`${pluralize(camelize("list"))}`).get(context.id, { headers: context.params.headers })
                            if(list && list.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('list')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let list = await app.service(`${pluralize(camelize("lists"))}`).get(context.id, { headers: context.params.headers })
                            if(list && list.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('list')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('List Rest Server on http://%s:%d', app.get('host'), port)
);
