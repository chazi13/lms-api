...
try {
    ...
    externalHook = require(path + '/hooks/event')
} catch (e) {
...
const eventService = new cote.Responder({
    name: 'Event Service',
    key: 'event'
})
...
eventService.on("find", async (req, cb) => {
    try {
        let data = await app.service("events").find({
...
eventService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("events").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
eventService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("events").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
eventService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("events").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
eventService.on("create", async (req, cb) => {
    try {
        let data = await app.service("events").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
eventService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("events").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
eventService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("events").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
eventService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("events").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('event')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('event')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let event = await app.service(`${pluralize(camelize("event"))}`).get(context.id, { headers: context.params.headers })
                            if(event && event.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('event')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let event = await app.service(`${pluralize(camelize("events"))}`).get(context.id, { headers: context.params.headers })
                            if(event && event.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('event')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Event Rest Server on http://%s:%d', app.get('host'), port)
);
