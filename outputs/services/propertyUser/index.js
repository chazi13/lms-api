...
try {
    ...
    externalHook = require(path + '/hooks/propertyUser')
} catch (e) {
...
const propertyUserService = new cote.Responder({
    name: 'PropertyUser Service',
    key: 'propertyUser'
})
...
propertyUserService.on("find", async (req, cb) => {
    try {
        let data = await app.service("propertyUsers").find({
...
propertyUserService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("propertyUsers").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
propertyUserService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("propertyUsers").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
propertyUserService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("propertyUsers").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
propertyUserService.on("create", async (req, cb) => {
    try {
        let data = await app.service("propertyUsers").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
propertyUserService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("propertyUsers").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
propertyUserService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("propertyUsers").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
propertyUserService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("propertyUsers").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('propertyUsers').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('propertyUser')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('propertyUser')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let propertyUser = await app.service(`${pluralize(camelize("propertyUser"))}`).get(context.id, { headers: context.params.headers })
                            if(propertyUser && propertyUser.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('propertyUser')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let propertyUser = await app.service(`${pluralize(camelize("propertyUsers"))}`).get(context.id, { headers: context.params.headers })
                            if(propertyUser && propertyUser.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('propertyUser')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('PropertyUser Rest Server on http://%s:%d', app.get('host'), port)
);
