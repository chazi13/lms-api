...
try {
    ...
    externalHook = require(path + '/hooks/checkInRoom')
} catch (e) {
...
const checkInRoomService = new cote.Responder({
    name: 'CheckInRoom Service',
    key: 'checkInRoom'
})
...
checkInRoomService.on("find", async (req, cb) => {
    try {
        let data = await app.service("checkInRooms").find({
...
checkInRoomService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("checkInRooms").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
checkInRoomService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("checkInRooms").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
checkInRoomService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("checkInRooms").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
checkInRoomService.on("create", async (req, cb) => {
    try {
        let data = await app.service("checkInRooms").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
checkInRoomService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("checkInRooms").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
checkInRoomService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("checkInRooms").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
checkInRoomService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("checkInRooms").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('checkInRooms').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('checkInRoom')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('checkInRoom')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let checkInRoom = await app.service(`${pluralize(camelize("checkInRoom"))}`).get(context.id, { headers: context.params.headers })
                            if(checkInRoom && checkInRoom.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('checkInRoom')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let checkInRoom = await app.service(`${pluralize(camelize("checkInRooms"))}`).get(context.id, { headers: context.params.headers })
                            if(checkInRoom && checkInRoom.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('checkInRoom')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('CheckInRoom Rest Server on http://%s:%d', app.get('host'), port)
);
