...
try {
    ...
    externalHook = require(path + '/hooks/attachment');
} catch (e) {
...
const attachmentService = new cote.Responder({
...
attachmentService.on("find", async (req, cb) => {
    try {
        let data = await app.service("attachments").find({
...
attachmentService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("attachments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
attachmentService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("attachments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
attachmentService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("attachments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
attachmentService.on("create", async (req, cb) => {
    try {
        let data = await app.service("attachments").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
attachmentService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("attachments").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
attachmentService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("attachments").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
attachmentService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("attachments").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('attachment')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('attachment')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let attachment = await app.service(`${pluralize(camelize("attachment"))}`).get(context.id, { headers: context.params.headers })
                            if(attachment && attachment.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('attachment')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let attachment = await app.service(`${pluralize(camelize("attachments"))}`).get(context.id, { headers: context.params.headers })
                            if(attachment && attachment.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('attachment')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Attachment Rest Server on http://%s:%d', app.get('host'), port)
);
