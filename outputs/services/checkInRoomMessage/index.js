...
try {
    ...
    externalHook = require(path + '/hooks/checkInRoomMessage')
} catch (e) {
...
const checkInRoomMessageService = new cote.Responder({
    name: 'CheckInRoomMessage Service',
...
checkInRoomMessageService.on("find", async (req, cb) => {
    try {
        let data = await app.service("checkInRoomMessages").find({
...
checkInRoomMessageService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("checkInRoomMessages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
checkInRoomMessageService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("checkInRoomMessages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
checkInRoomMessageService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("checkInRoomMessages").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
checkInRoomMessageService.on("create", async (req, cb) => {
    try {
        let data = await app.service("checkInRoomMessages").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
checkInRoomMessageService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("checkInRoomMessages").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
checkInRoomMessageService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("checkInRoomMessages").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
checkInRoomMessageService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("checkInRoomMessages").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('checkInRoomMessages').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('checkInRoomMessage')}:findOwn`)){
                    ...
                    await checkPermissions({
                        roles: ['admin', 'checkInRoomMessage']
                    })(context)
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('checkInRoomMessage')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let checkInRoomMessage = await app.service(`${pluralize(camelize("checkInRoomMessage"))}`).get(context.id, { headers: context.params.headers })
                            if(checkInRoomMessage && checkInRoomMessage.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('checkInRoomMessage')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let checkInRoomMessage = await app.service(`${pluralize(camelize("checkInRoomMessages"))}`).get(context.id, { headers: context.params.headers })
                            if(checkInRoomMessage && checkInRoomMessage.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('checkInRoomMessage')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('CheckInRoomMessage Rest Server on http://%s:%d', app.get('host'), port)
);
