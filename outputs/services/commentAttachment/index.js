...
try {
    ...
    externalHook = require(path + '/hooks/commentAttachment');
} catch (e) {
...
const commentAttachmentService = new cote.Responder({
    name: 'CommentAttachment Service',
...
commentAttachmentService.on("find", async (req, cb) => {
    try {
        let data = await app.service("commentAttachments").find({
...
commentAttachmentService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("commentAttachments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
commentAttachmentService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("commentAttachments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
commentAttachmentService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("commentAttachments").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
commentAttachmentService.on("create", async (req, cb) => {
    try {
        let data = await app.service("commentAttachments").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
commentAttachmentService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("commentAttachments").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
commentAttachmentService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("commentAttachments").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
commentAttachmentService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("commentAttachments").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
...
app.service('commentAttachments').hooks({
    before: {
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('commentAttachment')}:findOwn`)){
        ...
        create: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeCreate
                    if(context.data && context.data.subCommentId){
                        let belongsTo = await getRequester('subComment').send({ 
                            type: "get", 
                            id: context.data.subCommentId, 
                            headers:{
                                token: context.params.headers.authorization
                            }
                        })
                        if(!belongsTo){
                            throw Error("SubComment not found.")
                        }
                    }             
                    
                    if(context.data && context.data.commentId){
                        let belongsTo = await getRequester('comment').send({ 
                            type: "get", 
                            id: context.data.commentId, 
                            headers:{
                                token: context.params.headers.authorization
                            }
                        })
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('commentAttachment')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let commentAttachment = await app.service(`${pluralize(camelize("commentAttachment"))}`).get(context.id, { headers: context.params.headers })
                            if(commentAttachment && commentAttachment.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('commentAttachment')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let commentAttachment = await app.service(`${pluralize(camelize("commentAttachments"))}`).get(context.id, { headers: context.params.headers })
                            if(commentAttachment && commentAttachment.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('commentAttachment')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('CommentAttachment Rest Server on http://%s:%d', app.get('host'), port)
);
