...
try {
    ...
    externalHook = require(path + '/hooks/option')
} catch (e) {
...
const optionService = new cote.Responder({
    name: 'Option Service',
    key: 'option'
})
...
optionService.on("find", async (req, cb) => {
    try {
        let data = await app.service("options").find({
...
optionService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("options").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
optionService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("options").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
optionService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("options").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
optionService.on("create", async (req, cb) => {
    try {
        let data = await app.service("options").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
optionService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("options").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
optionService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("options").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
optionService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("options").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('option')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('option')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let option = await app.service(`${pluralize(camelize("option"))}`).get(context.id, { headers: context.params.headers })
                            if(option && option.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('option')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let option = await app.service(`${pluralize(camelize("options"))}`).get(context.id, { headers: context.params.headers })
                            if(option && option.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('option')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Option Rest Server on http://%s:%d', app.get('host'), port)
);
