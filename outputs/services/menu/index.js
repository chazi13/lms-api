...
try {
    ...
    externalHook = require(path + '/hooks/menu')
} catch (e) {
...
const menuService = new cote.Responder({
    name: 'Menu Service',
    key: 'menu'
})
...
menuService.on("find", async (req, cb) => {
    try {
        let data = await app.service("menus").find({
...
menuService.on("findConnection", async (req, cb) => {
    try {
        let data = await app.service("menus").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
menuService.on("findOwn", async (req, cb) => {
    try {
        let data = await app.service("menus").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
menuService.on("findConnectionOwn", async (req, cb) => {
    try {
        let data = await app.service("menus").find({
            query: transformer({where: req.where, limit: req.limit, skip: req.skip, orderBy: req.orderBy}),
            headers: req.headers,
...
menuService.on("create", async (req, cb) => {
    try {
        let data = await app.service("menus").create(req.body, {
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
        })
...
menuService.on("patch", async (req, cb) => {
    try {
        let data = await app.service("menus").patch(req.id, req.body, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
menuService.on("delete", async (req, cb) => {
    try {
        let data = await app.service("menus").remove(req.id, {
            ...req.params || {},
            headers: req.headers,
            file: req.file,
            isSystem: req.isSystem
...
menuService.on("get", async (req, cb) => {
    try {
        let data = null
        if (req.id) {
            data = await app.service("menus").get(req.id, {
                headers: req.headers,
                isSystem: req.isSystem
            })
        }
        ...
        find: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    if(auth.user.permissions.includes(`${camelize('menu')}:findOwn`)){
        ...
        update: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforeUpdate
                    if(auth.user.permissions.includes(`${camelize('menu')}:updateOwn`)){
                        context.method = "updateOwn"
                        if(context.id){
                            let menu = await app.service(`${pluralize(camelize("menu"))}`).get(context.id, { headers: context.params.headers })
                            if(menu && menu.createdBy !== auth.user.id){
        ...
        patch: async (context) => {
            try {
                if(!context.params.isSystem){
                    ...
                    //beforePatch
                    if(auth.user.permissions.includes(`${camelize('menu')}:patchOwn`)){
                        context.method = "patchOwn"
                        if(context.id){
                            let menu = await app.service(`${pluralize(camelize("menus"))}`).get(context.id, { headers: context.params.headers })
                            if(menu && menu.createdBy !== auth.user.id){
                                throw new Error("UnAuthorized")
                            }
                        }
        ...
        remove: async (context) => {
            try {
               if(!context.params.isSystem){
                    ...
                    //beforeDelete
                    if(auth.user.permissions.includes(`${camelize('menu')}:removeOwn`)){
...
server.on('listening', () =>
    console.log('Menu Rest Server on http://%s:%d', app.get('host'), port)
);
