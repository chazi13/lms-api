...
const { CountSuccessServices, CountFailServices } = require('../prom-client')
module.exports = class Requester extends Monitorable(Configurable(Component)) {
  ...
  onAdded(obj) {
    ...
    const alreadyConnected = this.sock.socks.some(s => (this.constructor.useHostNames ? s._host == obj.hostName : s.remoteAddress == address) && s.remotePort == obj.advertisement.port);
  ...
  send(...args) {
    ...
    return new Promise((resolve, reject) => {
      sendOverSocket(this.sock, timeout, ...args, (err, res) => {
        if (err){
          CountFailServices.labels(this.advertisement.name, args[0].type).inc(); 
        }else{
          CountSuccessServices.labels(this.advertisement.name, args[0].type).inc(); 
        }
